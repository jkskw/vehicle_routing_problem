...
def genetic_algorithm():
    ...
    u, route_init = (min(evaluate_fitness(chormosome) for chormosome in population))
...
def plot_routes(routes):
    ...
    for idx, route in enumerate(routes):
        ...
        # Generates text for summarizing demands of each route
    ...
    plt.scatter([loc[0] for loc in customer_locations], [loc[1] for loc in customer_locations], color='b', marker='s', label='Customers')
    plt.xlabel('X')
    plt.ylabel('Y')
...
def main():
    ...
    print("Initial length traveled:", init_length)
    
    # Plots the initial routes
    plot_routes(route_init)
    ...
    print("Total length traveled:", total_distance_traveled)
